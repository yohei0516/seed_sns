...
session_start();
  ...
  require('dbconnect.php');
      ...
      // 自分をフォローしている一覧データを取得
      $sql = "SELECT * FROM `members` INNER JOIN `follows` ON `members`.`member_id` = `follows`.`member_id` 
      ...
      while(1){
        ...
        if($one_tweet == false){
          ...
          // following_flagを用意して、自分もフォローしていたら1、していなかったら0を代入する
      ...
      if (isset($_GET["follow_id"])) {
        ...
        // フォロー押す前の状態に戻す(再読み込みで、再度フォロー処理が動くのを防ぐ)
